...
SharedPtr<Mesh> ParseMeshWavefront(const std::string& filename) {
  ...
  SharedPtr<Material> default_material =
      CreateShared<Material>(AssetRegistry::Instance()->Load<Shader>("assets/shaders/basic_normal_mapped.glsl"));
  ...
  default_material->AddTexture(AssetRegistry::Instance()->Load<Texture>("assets/textures/blank.png"), "{}.{}", kUniformNameMaterial,
...
bool ParseMaterialsWavefront(const std::string& filename, MapMaterials& materials_map) {
  ...
  for (uint32_t i = 0; i < materials_count; ++i) {
    SharedPtr<Shader> shader = AssetRegistry::Instance()->Load<Shader>("assets/shaders/deferred_geometry.glsl");
    ...
    if (materials[i].map_diffuse.has_value()) {
      ...
      material->AddTexture(AssetRegistry::Instance()->Load<Texture>("assets/textures/blank.png"), "{}.{}", kUniformNameMaterial,
                           kStructMemberNameDiffuseMap);
    }
